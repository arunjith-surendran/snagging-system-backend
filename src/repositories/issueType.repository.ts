 ...
 */
const getAllIssueTypes = async (): Promise<{ issueTypes: IIssueType[]; totalCount: number;}> => {
  const data = await db.select().from(issue_types);
  return { issueTypes: data as IIssueType[], totalCount: data.length };
};
